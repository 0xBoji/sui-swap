module SuiSwap::SuiSwap {
    ...
    public fun create_pool<CoinType1: drop, CoinType2: drop>(
        ...
        BasicCoin::publish_balance<CoinType1>(suiswap);
        BasicCoin::publish_balance<CoinType2>(suiswap);
        assert!(signer::address_of(suiswap) == @SuiSwap, error::invalid_argument(ESuiSwap_ADDRESS));
        assert!(!exists<LiquidityPool<CoinType1, CoinType2>>(signer::address_of(suiswap)), error::already_exists(EPOOL));
        move_to(suiswap, LiquidityPool<CoinType1, CoinType2>{coin1, coin2, share});

        // Transfer the initial liquidity of CoinType1 and CoinType2 to the pool under @SuiSwap.
        BasicCoin::transfer<CoinType1>(requester, signer::address_of(suiswap), coin1, witness1);
        BasicCoin::transfer<CoinType2>(requester, signer::address_of(suiswap), coin2, witness2);
    ...
    public fun coin1_to_coin2_swap_input<CoinType1: drop, CoinType2: drop>(
        ...
        assert!(signer::address_of(suiswap) == @SuiSwap, error::invalid_argument(ESuiSwap_ADDRESS));
        assert!(exists<LiquidityPool<CoinType1, CoinType2>>(signer::address_of(suiswap)), error::not_found(EPOOL));
        let pool = borrow_global_mut<LiquidityPool<CoinType1, CoinType2>>(signer::address_of(suiswap));
    ...
    public fun add_liquidity<CoinType1: drop, CoinType2: drop>(
        ...
        let pool = borrow_global_mut<LiquidityPool<CoinType1, CoinType2>>(@SuiSwap);
        ...
        BasicCoin::transfer<CoinType1>(account, @SuiSwap, coin1, witness1);
        BasicCoin::transfer<CoinType2>(account, @SuiSwap, coin2, witness2);
    ...
    public fun remove_liquidity<CoinType1: drop, CoinType2: drop>(
        ...
        BasicCoin::transfer<CoinType1>(suiswap, signer::address_of(requester), coin1_removed, witness1);
        BasicCoin::transfer<CoinType2>(suiswap, signer::address_of(requester), coin2_removed, witness2);
